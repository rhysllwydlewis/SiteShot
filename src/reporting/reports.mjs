...
function issueCard(issue, options = {}) {
  ...
  return `<article class="issue ${escapeHtml(issue.severity.toLowerCase())}"><div class="issue-meta"><strong>${escapeHtml(issue.severity)}</strong><span>${escapeHtml(issue.category)}</span><span>${escapeHtml(issue.module)}</span></div><h3>${escapeHtml(issue.title)}</h3><p>${escapeHtml(issue.description)}</p><p class="muted">${escapeHtml(issue.url)}</p>${showEvidence && issue.selector ? `<p><b>Selector:</b> <code>${escapeHtml(issue.selector)}</code></p>` : ''}${showEvidence && issue.evidence ? `<details><summary>Evidence</summary><pre>${escapeHtml(truncate(issue.evidence, 1800))}</pre></details>` : ''}${issue.screenshot ? `<p><a href="./${escapeHtml(issue.screenshot)}">Open evidence screenshot</a></p>` : ''}<p><b>Recommendation:</b> ${escapeHtml(issue.recommendation)}</p></article>`;
}

function htmlShell(title, body) {
  return `<!doctype html><html lang="en"><head><meta charset="utf-8"><title>${escapeHtml(title)}</title><meta name="viewport" content="width=device-width,initial-scale=1"><style>:root{font-family:Inter,system-ui,sans-serif;color:#10252c;background:#f2f7f7;--teal:#0f766e;--ink:#10252c;--muted:#60747b;--border:#dbe7e8}body{margin:0}.cover{padding:48px;background:linear-gradient(135deg,#0f766e,#10252c);color:#fff}.cover h1{font-size:44px;margin:0 0 10px}.nav{display:flex;gap:10px;flex-wrap:wrap;margin-top:18px}.nav a{color:white;background:rgba(255,255,255,.15);padding:8px 12px;border-radius:999px;text-decoration:none}main{max-width:1320px;margin:auto;padding:28px}.panel{background:white;border:1px solid var(--border);border-radius:26px;padding:24px;margin-bottom:22px}.overall{font-size:74px;font-weight:950}.grade{font-size:28px;font-weight:900;color:#0f766e}.scores{display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:12px}.score{border-radius:20px;padding:18px;border:1px solid var(--border)}.score span{display:block;color:var(--muted);font-size:13px}.score strong{font-size:34px}.good{background:#e9f8ee}.warn{background:#fff7e2}.bad{background:#fdecec}.stats{display:flex;gap:10px;flex-wrap:wrap}.pill{background:#e6f6f2;color:#0b4f4a;border-radius:999px;padding:9px 12px;font-weight:800;font-size:13px}.issue-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(330px,1fr));gap:14px}.issue{background:#fff;border:1px solid var(--border);border-radius:20px;padding:16px}.issue-meta{display:flex;gap:8px;flex-wrap:wrap}.issue-meta span,.issue-meta strong{font-size:12px;padding:5px 8px;border-radius:999px;background:#edf6f6}.muted{color:var(--muted);font-size:13px}pre{white-space:pre-wrap;background:#0d1b1f;color:#d9fff7;padding:12px;border-radius:12px;overflow:auto}table{width:100%;border-collapse:collapse;font-size:13px}th,td{border-bottom:1px solid #e4eeee;padding:10px;text-align:left;vertical-align:top}a{color:#0f766e}.small{font-size:13px;color:var(--muted)}.priority-list{display:grid;gap:12px}.priority-row{border:1px solid var(--border);border-radius:18px;padding:16px;background:#fff}@media(max-width:800px){.cover h1{font-size:32px}.overall{font-size:48px}}@media print{.nav{display:none}.panel{box-shadow:none;break-inside:avoid}.cover{background:#0f766e!important}}</style></head><body>${body}</body></html>`;
}

function supportNav(style = 'full') {
  const evidenceLinks = ['<a href="./gallery.html">Screenshot gallery</a>', '<a href="./fix-roadmap.html">Fix roadmap</a>', '<a href="./issues.csv">Issue CSV</a>', '<a href="./tickets.md">Developer tickets</a>'];
...
function htmlReport(manifest, issues, summary, variant = 'full') {
  ...
  if (style === 'technical') body += groupByCategory(priority).map(([cat, list]) => `<section class="panel"><h2>${escapeHtml(cat)}</h2><div class="issue-grid">${list.map(i => issueCard(i, { showEvidence: true })).join('')}</div></section>`).join('');
  else if (style === 'full') body += `<section class="panel"><h2>Full Issue Register</h2><table><thead><tr><th>Severity</th><th>Category</th><th>Issue</th><th>URL</th><th>Recommendation</th></tr></thead><tbody>${issueRows}</tbody></table></section>`;
  else body += `<section class="panel"><h2>Recommended next steps</h2><ol><li>Resolve Critical and High findings first.</li><li>Review the Technical Report with the development team.</li><li>Re-run the audit after fixes to confirm progress.</li></ol></section>`;
...
function roadmapHtml(manifest, issues, summary, selectedStyle = 'full') {
  ...
  const groups = [['Immediate', priorityIssues(issues).filter(i => ['Critical','High'].includes(i.severity))], ['Next sprint', priorityIssues(issues).filter(i => i.severity === 'Medium')], ['Backlog polish', priorityIssues(issues).filter(i => ['Low','Info'].includes(i.severity))]];
