...
- Added a proper NSIS Windows setup target.
- Added `npm run dist:installer` and `npm run installer:win`.
- Added setup packaging smoke checks into `npm run verify`.
- Windows build workflow now uploads a setup artifact and an unpacked fallback artifact.
- Release workflow now publishes the setup EXE as the recommended release download.
- Added setup guidance in `docs/INSTALLER.md`.
